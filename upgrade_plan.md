...
* **내용**: 브라우저의 `Web Speech API (TTS - Text to Speech)`를 활용하여, 카드를 뒤집을 때 자동으로 발음을 들려주거나 스피커 아이콘을 눌러 발음을 확인할 수 있는 기능을 추가합니다.
* **목적**: 시각적 학습을 청각적 학습으로 확장하여 실제 학습 효율을 극대화합니다.

### 3. 지능형 학습 알고즘 (SRS)

* **내용**: **SRS(Spaced Repetition System, 간격 반복 알고즘)** 개념을 도입합니다. (예: 틀린 단어는 더 자주, 맞춘 단어는 더 천천히 노출하는 Leitner System 방식)
