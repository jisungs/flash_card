...
#### 1.6 학습 세트 선택 기능 구현 (Card Set Selection) 🆕
**목표**: 사용자가 학습을 시작하기 전, 원하는 단어 세트(Set)를 선택하여 맞춤형 학습을 시작할 수 있는 기능 구현

- **데이터 구조 확장**:
    - `words.json`의 각 단어 객체에 `set` 속성 추가 (예: `"set": "기초 일본어"`, `"set": "비즈니스 영어"`).
    - 기존 `category`와는 별개로, 학습 목적에 따른 그룹화(Set)를 가능하게 함.

- **대시보드(index.html) UI 추가**:
    - **세트 선택 섹션**: "학습할 세트를 선택하세요"라는 문구와 함께 드롭다운(`select`) 메뉴 또는 세트 카드 리스트 구현.
    - **동적 버튼 연동**: 선택한 세트에 따라 "지금 바로 학습 시작하기" 버튼의 이동 경로를 `flash_card.html?set=세트명`으로 동적 변경.

- **학습 페이지(flash_card.html) 연동**:
    - **URL 파라미터 분석**: `URLSearchParams`를 통해 `set` 파라미터가 있는지 확인.
    - **세트 기반 필터링**: `set` 파라미터가 존재할 경우, `allWords`에서 해당 세트에 속한 단어들만 먼저 추출하여 `filteredWords`의 기본값으로 설정.
    - **기존 필터와 결합**: 세트 필터링 후, 사용자가 '오답 노트' 등을 선택하면 해당 세트 내에서 다시 필터링되는 계층적 필터 구조 구현.
